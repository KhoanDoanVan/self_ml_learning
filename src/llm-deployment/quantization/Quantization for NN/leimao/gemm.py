...
def quantization_int8(x, s, z):
    # signed int8 is asymmetric -> 256 values [-128,127]
    # Positive side: 1 … 127 → 127 values
    # Negative side: -1 … -128 → 128 values
...
def generate_quantization_constants(
        alpha,
        beta,
        alpha_q,
        beta_q
...
def quantization_matrix_multiplication_int8(
    ...
    Y_q_simulated = (z_Y + (s_b / s_Y)*(b_q.astype(np.int32) - z_b) + ((s_X*s_W) / s_Y)*(np.matmul(X_q.astype(np.int32), W_q.astype(np.int32))-(z_W*np.sum(X_q.astype(np.int32), axis=1, keepdims=True))-(z_X*np.sum(W_q.astype(np.int32), axis=0, keepdims=True))+(p*z_X*z_W)))
...
def main():
    ...
    s_X, z_X = generate_quantization_int8_constants(
        alpha=alpha_X,
        beta=beta_X
    )
    ...
    X_q = quantization_int8(
        x=X,
        s=s_X,
        z=z_X
    )
    X_q_dq = dequantization(
        x_q=X_q,
        s=s_X,
        z=z_X
    )
    ...
    s_W, z_W = generate_quantization_int8_constants(
        alpha_W,
        beta_W
    )
    ...
    W_q = quantization_int8(
        x=W,
        s=s_W,
        z=z_W
    )

    W_q_dq = dequantization(
        x_q=W_q,
        s=s_W,
        z=z_W
    )
    ...
    s_b, z_b = generate_quantization_int8_constants(
        alpha_b,
        beta_b
    )
    ...
    b_q = quantization_int8(
        x=b,
        s=s_b,
        z=z_b
    )

    b_q_dq = dequantization(
        x_q=b_q,
        s=s_b,
        z=z_b
    )
    ...
    s_Y, z_Y = generate_quantization_int8_constants(
        alpha_Y,
        beta_Y
    )
    ...
    Y_q_expected = quantization_int8(
        x=Y_expected,
        s=s_Y,
        z=z_Y
    )
    ...
    Y_expected_prime_q = quantization_int8(
        x=Y_expected_prime,
        s=s_Y,
        z=z_Y
    )
    Y_expected_prime_q_dq = dequantization(
        x_q=Y_expected_prime_q,
        s=s_Y,
        z=z_Y
    )
